...
const Navbar = ({ onMenuToggle, menuOpen }) => {
  ...
  // Badge styling keeps role context visible in every dashboard.
  const roleLabel = {
    [ROLE.ORG_ADMIN]: { text: 'ORG ADMIN', cls: 'bg-violet-100 text-violet-700' },
    [ROLE.SUPER_ADMIN]: { text: 'SUPER ADMIN', cls: 'bg-rose-100 text-rose-700' },
    [ROLE.RECRUITER]: { text: 'RECRUITER', cls: 'bg-sky-100 text-sky-700' },
    [ROLE.CANDIDATE]: { text: 'CANDIDATE', cls: 'bg-emerald-100 text-emerald-700' },
  ...
  const profileLink = user?.role === ROLE.CANDIDATE ? '/dashboard?editProfile=1' : '/dashboard';
